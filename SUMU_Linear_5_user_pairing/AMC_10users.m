function capacity=sim_MUCB10user(ctrlpar)
...
%         [max_C, pair_select] = max([C1 C2./2 C3./3 C4./4]);
        ...
        [max_C1, pair_select1] = max(C1);
        [max_C2, pair_select2] = max(C2);
        [max_C3, pair_select3] = max(C3);
        
        SINR1user(drop,:) = SINR1(pair_select1,:);
        SINR2user(drop,:) = SINR2(pair_select2,:);
        SINR3user(drop,:) = SINR3(pair_select3,:);
        
%         [max_C4, pair_select4] = max(C4);max_C4
