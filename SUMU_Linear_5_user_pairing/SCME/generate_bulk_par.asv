...
% if bandwidth extension(范围) is used
if strcmpi(IntraClusterDsUsed,'yes')
...
% step 3: determine DS（DELAY SPREAD）, AS(ANGLE SPREAD=AZIMUTH SPREAD) and SF(LOGNORMAL SHADOW FADING RANDOM VARIABLE) for all users
% This step takes into account channel scenario automatically
...
if strcmpi(IntraClusterDsUsed,'no')
    if (DelaySamplingInterval>0)
        taus_rounded=DelaySamplingInterval*floor( taus/DelaySamplingInterval + 0.5);%加0.5是为了四舍五入
    else
        taus_rounded=taus;
    end
else
