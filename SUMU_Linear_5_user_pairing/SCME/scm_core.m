...
if strcmp(scmpar.UniformTimeSampling,'yes')
    ...
    delta_t = (wavelength ./ linkpar.MsVelocity.')./2/scmpar.SampleDensity; %wavelength ./ linkpar.MsVelocity.'相干时间
end
t = repmat(delta_t,1,T).*repmat([0:T-1]+offset_time,K,1); % matrix containing the time axes（时间轴） for all links [KxT]
