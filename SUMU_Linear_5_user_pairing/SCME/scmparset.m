...
%     supports only six paths. The delays and mean AoD离开角度/AoAs到达角度 are determined
...
scmpar=struct(  'NumBsElements',8,...                  %基站天线阵列的元素个数 
                'NumMsElements',2,...                  %手机天线阵列的元素个数
                'Scenario','urban_micro',...           %仿真场景
                'SampleDensity', 2,...                  % 采样密度in samples/half-wavelength
                'NumTimeSamples',100,...                %采样点数
                'UniformTimeSampling','no',...           
                'BsUrbanMacroAS','eight',...            % choices: 'eight' and 'fifteen'. 
                'NumPaths',6,...                        %径数
                'NumSubPathsPerPath',20,...             % 每一径的子径数 only value supported is 20.
                ...
                'CenterFrequency',2e9,...               % 载频in Herz
                'ScmOptions','none',...                 % 'none','polarized','los','urban_canyon'
                'DelaySamplingInterval',5e-9,...        %时延采样间隔
                'PathLossModelUsed','no',...            %路径损耗是否包含在信道矩阵中
                'ShadowingModelUsed','no',...           %衰落是否包含在信道矩阵中
