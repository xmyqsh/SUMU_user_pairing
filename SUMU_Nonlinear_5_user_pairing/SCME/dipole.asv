...
%   the counter clock-wise(逆时针) angle (in degrees) seen 
...
if (nargin>1)
    slant=varargin{2};
    slant=-slant/180*pi;    % change sign为什么要改变符号？？？
else
    slant=0;
end

% put all angles（角度） to radians（弧度）
az=az/180*pi;
...
[X Y Z]=sph2cart(az_vec, zeros(size(az_vec)), repmat(1,size(az_vec)));%将球面坐标系转换为笛卡尔坐标系，[x,y,z] = sph2cart(THETA,PHI,R)

% rotation matrix(旋转矩阵) in cartesian coordinates
...
patternV(I)=0;
patternH(I)=0;
Inot=setdiff([1:numel(el)],I);%c = setdiff(A, B)  returns the values in A that are not in B
