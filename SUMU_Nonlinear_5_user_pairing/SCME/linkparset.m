...
%   uniformly(均匀地) distributed in a circular disk（圆盘） over [RMIN,RMAX] meters. RMIN
...
%     respect to north. This parameter is currently redundant（多余的）.
...
%   users are uniformly(一致地，均匀地) (in area) distributed in a cell with radius RMAX.

%   Authors: Jari Salo (HUT), Marko Milojevic (TUI) 
%   $Revision: 0.1$  $Date: Sep 30, 2004$

% create random variables from triangular（三角形的；三人间的） pdf whose width is 2*rmax
a=sum(rmax*rand(2,num));%rand产生0到1之间均匀分布的随机数  
