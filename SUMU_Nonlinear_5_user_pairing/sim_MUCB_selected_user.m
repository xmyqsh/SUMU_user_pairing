...
eval(['save ','F:\efijlmj\zhongxing\results/result_nonlinear_users_', num2str(length(user_selected)), '_', 'ebno', '_', num2str(10*log10(ebno_range)), 'dB.mat']);
toc
rmpath('SCME');
end
...
%                 H{k} = [H_u1(:,:,k); H_u2(:,:,k); H_u3(:,:,k); H_u4(:,:,k)];
                ...
                % sort users
                [sorted_error_H, sorted_user] = sort_users(error_H, num_user2paired);
                
            
                [L,Q,G,PPP]= GMD_encoder(sorted_error_H,num_user2paired,syspar.Nr*ones(num_user2paired,1));
                [r(m,:), num_rank(m,:)] = unique_r(diag(G), num_user2paired);
                SINR(m,:) = (ebno_range.*r(m,:).^2)./sum(num_rank(m,:));
                R_temp = zeros(1,numUsers);
                R_temp(1,[uepair(m,[sorted_user])]) = log2(1+SINR(m,:)).*num_rank(m,:);
                R = [R; R_temp];
                sorted_uepair = [sorted_uepair; uepair(m,[sorted_user])];
...
%                 end
                ...
                signel_power = [SINR(m,1)*ones(num_rank(m,1),1); SINR(m,2)*ones(num_rank(m,2),1)];
%                 if sum(num_rank(m,:)) ~= 4
%                     delta_error([2*find(num_rank(m,1:end-1) ~= 2)]) = [];
%                 end
                
                if sum( size(signel_power) == size(delta_error') ) ~= 2
                    signel_power
                    delta_error
                    pause;
                end
...
%                 end
                ...
                signel_power = [SINR(m,1)*ones(num_rank(m,1),1); SINR(m,2)*ones(num_rank(m,2),1); SINR(m,3)*ones(num_rank(m,3),1)];
%                 if sum(num_rank(m,:)) ~= 6
%                     delta_error([2*find(num_rank(m,1:end-1) ~= 2)]) = [];
%                 end
...
%                 end
                ...
                signel_power = [SINR(m,1)*ones(num_rank(m,1),1); SINR(m,2)*ones(num_rank(m,2),1); SINR(m,3)*ones(num_rank(m,3),1); SINR(m,4)*ones(num_rank(m,4),1)];
%                 if sum(num_rank(m,:)) ~= 8
%                     delta_error([2*find(num_rank(m,1:end-1) ~= 2)]) = [];
%                 end
